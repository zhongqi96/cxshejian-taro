...
var _typeof =
  ...
  eventHandle = {
    event: function () {
      var e =
        arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {};
      if (!onLaunchFn._inited)
        return void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
    ...
    pullDownRefresh: function () {
      if (!onLaunchFn._inited)
        return void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
      (TDSDK || customConf.config.autoOnPullDownRefresh) &&
        Util.eventHandle('WeappPullDownRefresh');
    },
    reachBottom: function () {
      if (!onLaunchFn._inited)
        return void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
      (TDSDK || customConf.config.autoOnReachBottom) &&
        Util.eventHandle('WeappReachBottom');
    },
    setAccount: function () {
      var e =
        arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {};
      return onLaunchFn._inited
        ? e.accountId || /0{1}/.test(e.accountId)
        ...
        : void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
    },
  ...
  appHandle = {
    ...
    show: function () {
      ...
      if (!onLaunchFn._inited)
        return void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
      if (appHandle.appCount % 2 == 1)
        return void console.error(
          'TDSDK.App.show()和TDSDK.App.hide()应该成对调用'
        );
      if (
        ((appHandle.appCount = appHandle.appCount + 1),
  ...
  pageHandle = {
    ...
    hide: function () {
      if (!onLaunchFn._inited)
        return void console.error('请先调用TDSDK.App.onLaunch()完成初始化');
      if (0 == pageHandle.pageTime)
        return void console.error(
          'TDSDK.Page.onShow()和TDSDK.Page.onHide应该成对调用'
        );
  ...
  TDSDK = {
    ...
    Event: {
      ...
      setAccount: eventHandle.setAccount,
      setWxAccount: eventHandle.setWXUserInfo,
    },
  };
